📚 论文检阅 (AI Academic Assistant) - 基于Dify工作流
论文检阅 (AI Academic Assistant) 是一个基于 Dify 平台构建的智能体工作流 (DSL)。

本项目能够根据用户的自然语言对话，自动分发至 6 大专属处理分支。从多源数据库的并行检索、相似文献交叉推荐、引文网络演进分析、全文本深度精读、到最终的 Zotero 自动化库管。
...
Jina Reader API (r.jina.ai)：用于将 URL 转换为大模型友好的 Markdown。

Zotero API Key & User ID：用于激活 Save 自动化入库功能。

JSONBin API Key：用于激活引文知识图谱托管功能。

可选：Semantic Scholar API Key（免 Key 亦可使用，但配置 Key 可提高并发检索的稳定性与速率上限）。

🚀 部署与使用 (Getting Started)
第一步：导入工作流
下载本项目中的 dify-academic-workflow.yml 文件。

登录您的 Dify 工作台。

点击 "创建应用" -> 选择 "导入 DSL 文件"。
...
🗺️ 工作流全景图 (Workflow Topology)
(建议在此处插入您 Dify 工作流的全局截图，展示复杂的 6 分支路由架构)
